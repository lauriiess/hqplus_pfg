...
class ClinicDirectoryScreen extends StatefulWidget {
  ...
  @override State<ClinicDirectoryScreen> createState() => _ClinicDirectoryScreenState();
}
...
class _ClinicDirectoryScreenState extends State<ClinicDirectoryScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) setState(() { _clinics = list; _filtered = list; _loading = false; });
    } catch (_) {
      // Fallback to static
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Find a Clinic', style: TextStyle(fontWeight: FontWeight.w900))),
        ...
        Expanded(child: _loading
          ...
          : RefreshIndicator(onRefresh: _load, child: ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                final queue = c['queueLength'] ?? 0;
                final busy = (queue as int) > 8;
                    ...
                    child: Row(children: [
                      ...
                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                        Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                          decoration: BoxDecoration(
                            color: busy ? AppColors.error.withOpacity(0.1) : AppColors.success.withOpacity(0.1),
                            borderRadius: BorderRadius.circular(99)),
                          child: Text('\${wait}m',
                            style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                              color: busy ? AppColors.error : AppColors.success))),
                        const SizedBox(height: 2),
                        Text('\$queue queued', style: const TextStyle(fontSize: 10, color: AppColors.textMuted)),
                      ]),
                    ])));
              }))),
      ]),
