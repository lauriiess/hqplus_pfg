...
const joinQueue = async (req, res) => {
  try {
    ...
    const entry = await QueueEntry.create({
      ...
      estimatedWaitTime: estWait, peopleAhead: activeCount,
      joinedAt: new Date(),
    });
    ...
    return res.status(201).json({
      ...
      entry: {
        _id: entry._id, ticketNumber: queueNumber,
        clinicName: clinic.name, clinicAddress: clinic.address,
        serviceName, status: 'waiting',
        estimatedWaitTime: estWait, peopleAhead: activeCount,
...
// ─── Patient checks own status ────────────────────────────────────────────────
const getMyQueueStatus = async (req, res) => {
  try {
    const todayStart = new Date(); todayStart.setHours(0,0,0,0);
    const entry = await QueueEntry.findOne({
      patient: req.user._id, status: { $in: ['waiting','serving'] },
    ...
    const activeCount = await QueueEntry.countDocuments({
      clinic: entry.clinic._id, status: { $in: ['waiting','serving'] }, joinedAt: { $gte: todayStart },
    });

    return res.json({
      active: true,
      entry: {
        _id: entry._id, ticketNumber: entry.queueNumber,
        clinicName: entry.clinic?.name || '', clinicAddress: entry.clinic?.address || '',
        serviceName: entry.serviceName, status: entry.status,
...
const callPatient = async (req, res) => {
  try {
    const entry = await QueueEntry.findByIdAndUpdate(req.params.id, { status: 'serving' }, { new: true });
